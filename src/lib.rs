...
impl Sheet {
    pub fn cell_ref(&self, x: u16, y: u16) -> Option<&Cell> {
    ...
    pub fn get_display(&self, x: u16, y: u16) -> String {
        self.cell_ref(x, y).unwrap().display(self)
    }
    pub fn recalculate(&mut self) {
        for ((row, col), c) in self.array.indexed_iter_mut() {
            if let CellData::Function(s, None) = c.data.clone() {
                let mut s2 = String::new();
                s2.push_str(&s);
                c.data = CellData::from_raw(&s2, (col as u16 + 1, row as u16 + 1));
            }
        }
    }
    pub fn edit_cell<W: Write>(&mut self, stdout: &mut W, curpos: (u16, u16), scrollpos: (u16, u16)) -> Result<()> {
        ...
        if let Some(curx) = curx {
            if let Some(cury) = cury {
                if curx >= 1 && cury >= 1 {
                    let mut buf = self.cell_ref(cx, cy).unwrap().data.as_raw();
...
impl Context {
    ...
    fn open_sheet(&mut self, filename: Option<&str>) -> Result<()> {
    ...
    fn write_sheet(&mut self, filename: Option<&str>) -> Result<()> {
        if let Some(filename) = filename {
            self.filename.clear();
            self.filename.push_str(filename);
        }
...
fn handle_command(context: &mut Context, command_str: &str) -> Result<String> {
    ...
    if let Some(captures) = RE.captures(&command_str) {
        ...
        Ok(format!("Not a command \"{}\"", &command_str))
    }
}

pub fn run<W: Write>(mut stdout: &mut W) -> Result<()> {
    println!("yeet");
